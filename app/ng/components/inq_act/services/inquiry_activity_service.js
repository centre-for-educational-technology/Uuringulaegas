...
.factory("InquiryActivityService", function InquiryActivityFactory($resource, $http, appConfig, $rootScope){
    ...
    return $resource(appConfig.apiUrl + 'wp-json/pods/inq_activity/:id', {}, {
      ...
      getActivitiesBackgroundImage: {
        url: appConfig.apiUrl + 'wp-admin/admin-ajax.php',
        method: 'GET',
        params: {
          action: 'get_activities_background_image'
        },
      ...
      fullSearch: {
        url: appConfig.apiUrl + 'wp-json/pods/inq_activity?data[where]=(:query)&data[page]=:page&data[limit]=:limit&data[pagination]=true&data[orderby]=id+DESC',
        query: '@query',
        method: 'GET',
        isArray: true,
        transformResponse: [angular.fromJson, function(data, headers){
